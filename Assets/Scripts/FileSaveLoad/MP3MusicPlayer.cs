...
public class MP3MusicPlayer : MonoBehaviour {
    ...
    IEnumerator SimpleFileBrowserLoadCoroutine() {
        yield return SimpleFileBrowser.FileBrowser.WaitForLoadDialog(FileBrowser.PickMode.Folders, false, musicFolderPath, null, "Select Music MP3 Folder", "Select Folder" );
        if(SimpleFileBrowser.FileBrowser.Success) { 
            musicFolderPath = SimpleFileBrowser.FileBrowser.Result[0]; 
    ...
    IEnumerator SimpleFileBrowserPlayCoroutine() {
        yield return SimpleFileBrowser.FileBrowser.WaitForLoadDialog(FileBrowser.PickMode.Files, false, musicFolderPath, null, "Select Music MP3 File", "Play MP3" );
        if(SimpleFileBrowser.FileBrowser.Success) { PlayMP3FromPath(SimpleFileBrowser.FileBrowser.Result[0]); }
        musicFolderBackground.SetActive(false);
    }
